...
gulp.task('css', function compileStyle() {
...
gulp.task('js', function compileJS() {
  ...
  bundler.bundle().on('error', function onError(err) {
    console.error('[browserify error]', err.message);
  }).pipe(source('bundle.js'))
    .pipe(gulp.dest('dist/'));
});

gulp.task('serve', ['css', 'js'], function serve() {
  ...
  gulp.watch(['src/server/**/*.js'], function watchJS() {
    server.start.bind(server)();
  });

  // Notify server when frontend files change.
  gulp.watch(['dist/**/*.{css,js,html}'], function watchDist(file) {
    server.notify(file);
  });
});
